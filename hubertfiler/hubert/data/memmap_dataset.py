...
from .utils import get_bytes_range, file_size

PathOrStr = Union[str, PathLike]

class MemMapDataset(Dataset[Dict[str, Any]]):
    """
    A PyTorch :class: '~torch.utils.data.Dataset` made from numpy memory-mapped arrays
    of tokens. Tokens are chunked together in contiguous blocks of memory.
    Implementation largely adapted from Groenveld et. al 2024
    ...
    def __init__(
        ...
        if generate_attention_mask and pad_token_id is None:
            raise ValueError("Must give pad_token_id to generate atttention mask")
        
        if isinstance(metadata, list):
            if len(metadata) != len(paths):
                raise ValueError("metadata should have same length as file paths")
        else:
