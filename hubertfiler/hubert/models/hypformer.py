...
class TransConvLayer(nn.Module):
    def __init__(self, manifold, in_channels, out_channels, num_heads, use_weight=True, args=None):
        ...
        if self.attention_type == 'linear_focused':
            # Only make parameters if they'll be used for faster DDP training 
    ...
    def full_attention(self, qs, ks, vs, output_attn=False, mask=None):
        # normalize input
        ...
        if mask is not None:
            att_weight = att_weight.masked_fill(mask==0, -1e9)
    ...
    def linear_focus_attention(self, hyp_qs, hyp_ks, hyp_vs, output_attn=False):
            ...
            # Batch-aware kernel-transformed computations
            # phi_ks, v: [B, N, H, D]
            # Step 1: sum over N per batch & head -> [B, H, D, D]
            k_transpose_v = torch.einsum('bnhm,bnhd->bhmd', phi_ks, v)

            # Step 2: dot Q with that -> [B, N, H, D]
            numerator = torch.einsum('bnhm,bhmd->bnhd', phi_qs, k_transpose_v)

            # Step 3: normalizer as sum of K over N -> ks_sum: [B, H, D]
            ks_sum = torch.einsum('bnhd->bhd', phi_ks)
            # denominator: [B, N, H, 1]
            denominator = torch.einsum('bnhd,bhd->bnh', phi_qs, ks_sum).unsqueeze(-1)
        ...
        if output_attn:
            if self.attention_type == 'linear_focused':
                ...
                attention_output, attn = self.full_attention(
                    query, key, value, output_attn, mask=mask)
            else:
...
class HypFormer(nn.Module):
    ...
    def forward(self, x, mask=None):
        x1 = self.trans_conv(x, mask=mask)
        if self.decoder_type == 'euc':
